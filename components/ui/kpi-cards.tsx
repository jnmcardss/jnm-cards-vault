...
function pctGain(invested: number, value: number) {
...
export function KpiCards() {
  ...
  // Prevent SSR/CSR mismatch when totals come from client-only storage
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  if (!mounted) return null;

  const gain = useMemo(
    () => pctGain(totals.totalInvested, totals.collectionValue),
    [totals.totalInvested, totals.collectionValue]
  );
  ...
  return (
    <div className="grid gap-6 md:grid-cols-4">
      ...
      <Kpi
        ...
        value={money(totals.totalInvested)}
        icon={ShoppingCart}
      />
